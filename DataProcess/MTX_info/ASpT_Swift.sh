...
file_a="results_Swift_FP32_128.txt"
file_b="result_ASpT_spmm_f32_n128.txt"
file_c="MTX_info.txt"
output_file="ASpT_Swift.txt"
...
while read lineA <&3 && read lineB <&4; do
    ...
    col7=$(echo "$lineA" | awk '{print $9}') #nnz
    col8=$(echo "$lineA" | awk '{print $13}') #Swift time

    col5B=$(echo "$lineB" | awk '{print $5}') #ASpT time
    col6B=$(echo "$lineB" | awk '{print $6}') #ASpT gflops
    echo "$col2 $col7 $col5B $col8" >> $output_file3
...
while read lineA <&3 && read lineB <&4; do
    ...
    col3=$(echo "$lineA" | awk '{print $3}') #ASpT time
    col4=$(echo "$lineA" | awk '{print $4}') #Swift time


    col11B=$(echo "$lineB" | awk '{print $15}') #blk_ratio


    echo "$col1 $col2 $col11B $col3 $col4" >> $output_file8
done 3< $output_file7 4< $output_file6


awk '$3 != 0.000000' $output_file8 > tmp && mv tmp $output_file8

awk '{if($5!=0) res=$4/$5; else res="N/A"; print $0, res}' $output_file8 > $output_file
