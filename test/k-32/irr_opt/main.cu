...
time_balance = (tbalance2.tv_sec - tbalance1.tv_sec) * 1000.0 + (tbalance2.tv_usec - tbalance1.tv_usec) / 1000.0;
    ...
    if (time_Swift_gpu1 != -1 && time_Swift_gpu2 != -1)
    {

        printf("Swift GPU SpMM irregular part without optimization: %f ms\n", time_Swift_gpu);
        printf("Swift GPU SpMM irregular part with optimization: %f ms\n", time_Swift_gpu2);  
        FILE *fout = fopen(RESULT_FILE_PATH, "a");
        if (fout == NULL)
            printf("Writing results fails.\n");
        fprintf(fout, "%s m %d n %d width %d nnz %d non-opt %f opt %f\n",
                filename,rowA, colA, BN, nnz,time_Swift_gpu, time_Swift_gpu2);
        fclose(fout);
    }
    else
    {
