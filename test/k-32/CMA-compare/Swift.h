        ...
        case 2:
        {
            ...
            for(int i = 0; i <count; i++)
            {
               ...
               /*
                ...
                // 计算 warp 级别前缀和
                shared_mem[threadIdx.x] = warp_prefix_sum(val);
                __syncthreads();
                int judge = d_sortrowindex[tile_start + lane_id];
                if(judge !=0)
                {
                    scaled_val = shared_mem[threadIdx.x+judge]-shared_mem[threadIdx.x] + val_tmp;
                    atomicAdd(&d_resultmtx[result_index], scaled_val);
                } 
                    */            
...
//    cudaStream_t stream1, stream2;
 ...
 //   cudaStreamCreate(&stream2);    
    ...
    if(tilenum !=0 && h_reside_nnz !=0)
    {
        ...
        Swift_GPU<<<tilenum, 1024>>>(rowA, colA,  
                                               width, nnz,
                                               tilenum,
                                               d_tile_ptr,
                                               d_tile_colidx,
    ...
    size_t total_size1 = 4 * tilenum * 2 + 4 *(tilenum +1) + regular_nnz * 4 + regular_nnz * 8 + 8 * width * colA + 8 * width * rowA;
    size_t total_size2 = (h_reside_col+1) * 4 + h_reside_nnz *4 + h_reside_nnz *8 + d_numblk * 4 * 4;
    size_t total_size = total_size1 + total_size2;
    float time_sec = time_Swift_gpu / 1000.0;
    float bandwidth = total_size / (time_sec * 1e9);
    //printf("Swift GPU SpMM bandwidth = %f GB/s\n", bandwidth);
    ...
    {
        //printf("Check Swift GPU SpMM PASS!\n");
        printf("Swift GPU SpMM (column-major, with CMA) = %f ms\n",time_Swift_gpu);

        
    }
    else
    {
        printf("Swift GPU SpMM (column-major, with CMA) Check NO PASS! error = %d \n", error_Swift_spmm);
        time_Swift_gpu = -1;
