...
void Swift_row_major(char *filename,
...
else if(tilenum ==0 && h_reside_nnz !=0)
{
    ...
    if (error_Swift_spmm == 0)
    {
        ...
        printf("Swift GPU SpMM (row-major, without CMA) time = %f ms\n",time_Swift_gpu);

        
    }
    else
    {
        printf("Swift GPU SpMM (row-major, without CMA) Check NO PASS! error = %d \n", error_Swift_spmm);
        time_Swift_gpu = -1;
