...
__global__ void FastLoad_SpMM_kernel2(int rowA, int colA, int width, int nnz,
                                      int tilenum,
                                      int *d_tile_ptr,
                                      //int *d_tile_tall,
                                      int *d_tile_len,
...
__global__ void FastLoad_SpMM_reside_kernel2(int rowA, int colA, int width, int nnz,
                                            int reside_col,
                                            int *d_reside_ptr,
                                            //int *d_tile_tall,
                                            int *d_reside_rowidx,
                                            ...
                                            float *d_resultmtx)
{
    ...
    if(th_tile < reside_col)
    {
        ...
        for(int i = d_start + lane_id; i < d_stop ; i+=32)
        {
            ...
            {
                float d_dn_value = d_densemtx[colidx_index + j*colA];
...
__global__ void FastLoad_SpMM_reside_kernel1(int rowA, int colA, int width, int nnz,
                                            int reside_col,
                                            int *d_reside_ptr,
                                            //int *d_tile_tall,
                                            int *d_reside_rowidx,
...
void FastLoad_gpu1(char *filename,
...
//reside part
    ...
    int num_blocks_reside = ceil((float)h_reside_col / (float)warpperblock);
...
if(tilenum !=0 && h_reside_nnz !=0)
{
    ...
    cudaEventRecord(event1,0);
        ...
        FastLoad_SpMM_kernel2<<<tilenum, 1024,0,stream1>>>(rowA, colA, width, nnz,
                                                            tilenum,
                                                            d_tile_ptr,
                                                        //int *d_tile_tall,
                                                            d_tile_len,
        ...
        FastLoad_SpMM_reside_kernel1<<<num_blocks_reside, num_thread_reside,0,stream2>>>(rowA, colA, width, nnz,
                                                                              h_reside_col,
                                                                              d_reside_ptr,
...
else if(tilenum !=0 && h_reside_nnz ==0)
{
    ...
    cudaEventRecord(event1,0);
       ...
       FastLoad_SpMM_kernel2<<<tilenum, 1024>>>(rowA, colA, width, nnz,
                                                tilenum,
                                                d_tile_ptr,
                                                d_tile_len,
                                                d_tile_colidx,
...
else if(tilenum ==0 && h_reside_nnz !=0)
{
    ...
    //cudaDeviceSynchronize();
    cudaEventRecord(event1,0);         
    FastLoad_SpMM_reside_kernel1<<<num_blocks_reside, num_thread_reside>>>(rowA, colA, width, nnz,
                                                                              h_reside_col,
                                                                              d_reside_ptr,
    ...
    {
        printf("Swift GPU SpMM (dense mtx col-major,no balance) Check NO PASS! error = %d \n", error_FastLoad_spmm);
        time_fastload_gpu = -1;
    }
