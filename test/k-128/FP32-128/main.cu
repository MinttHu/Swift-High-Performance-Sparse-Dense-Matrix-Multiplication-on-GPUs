...
time_transform = (ttransform2.tv_sec - ttransform1.tv_sec) * 1000.0 + (ttransform2.tv_usec - ttransform1.tv_usec) / 1000.0;
    ...
    float gflops_fastload_gpu=0;
    float time_fastload_gpu1=0;
    float gflops_fastload_gpu1=0;
    float time_fastload_gpu2=0;
    float gflops_fastload_gpu2=0;
    memset(result_mtx,0, sizeof(float) * rowA*BN);     
...
time_balance = (tbalance2.tv_sec - tbalance1.tv_sec) * 1000.0 + (tbalance2.tv_usec - tbalance1.tv_usec) / 1000.0;
    ...
    memset(result_mtx,0, sizeof(float) * rowA*BN);
    float time_fastload_gpu3=0;
    float gflops_fastload_gpu3=0;
    Swift_GPU_3(filename,
              time_fastload_gpu3,
              gflops_fastload_gpu3,
              matrixA,
              rowA,
              colA,
              BN,
    ...
    float time_fastload_gpu4=0;
    float gflops_fastload_gpu4=0;
    Swift_GPU_4(filename,
              time_fastload_gpu4,
              gflops_fastload_gpu4,
              matrixA,
              rowA,
              colA,
              BN,
    ...
    float time_final1;
    result_check(time_final, time_fastload_gpu3, time_fastload_gpu4, time_final1);    
    ...
    else
    {
        ...
        if (fout == NULL)
            printf("Writing results fails.\n");
        fprintf(fout, "erro FastLoad (dense mtx col-major)%s \n",
                       filename );
        fclose(fout);
    }
