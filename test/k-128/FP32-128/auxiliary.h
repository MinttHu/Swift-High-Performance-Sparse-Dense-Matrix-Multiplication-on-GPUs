...
#define RESULT_FILE_PATH "./data/results_Swift_FP32_128.txt"
float min_val(float a, float b, float c) {
    return fminf(a, fminf(b, c));
}

void result_check(float time_fastload_gpu, float time_fastload_gpu1, float time_fastload_gpu2, float &time_final)
{
    if(time_fastload_gpu ==0)
    {
        time_fastload_gpu = -1;
    }

    if(time_fastload_gpu1 ==0)
    {
        time_fastload_gpu1 = -1;
    }
    if(time_fastload_gpu2 ==0)
    {
        time_fastload_gpu2 = -1;
    }   
    
    if(time_fastload_gpu !=-1 && time_fastload_gpu1 !=-1 && time_fastload_gpu2 !=-1)
    {
        time_final = fminf(time_fastload_gpu, fminf(time_fastload_gpu1, time_fastload_gpu2));
    }
    else if(time_fastload_gpu ==-1 && time_fastload_gpu1 !=-1 && time_fastload_gpu2 !=-1)
    {
        time_final = fminf(time_fastload_gpu1, time_fastload_gpu2);
    }
    else if(time_fastload_gpu !=-1 && time_fastload_gpu1 ==-1 && time_fastload_gpu2 !=-1)
    {
        time_final = fminf(time_fastload_gpu, time_fastload_gpu2);
    }
    else if(time_fastload_gpu !=-1 && time_fastload_gpu1 !=-1 && time_fastload_gpu2 ==-1)
    {
        time_final = fminf(time_fastload_gpu, time_fastload_gpu1);
    }
    else if(time_fastload_gpu ==-1 && time_fastload_gpu1 ==-1 && time_fastload_gpu2 ==-1)
    {
        time_final = -1;
...
void dense_mtx2dense_mtx_spmm(int rows, int cols, float *originmtx, float *targetmtx)
{
  int k=0;
...
void auxilary_print_mtx(int row, int col,int width, float *mtx)
{
    for(int i=0; i< row; i++)
    {
        for(int j=0; j<col; j++)
...
void generateSparseVector(float sparsity, int size, float *vector) {
    srand(1);

    for (int i = 0; i < size; ++i) {
        float randomValue = (rand() % 10 + 1) * 1.0;
...
void Golden_y(int rowA, int colA, int *csrPtr, int *csrColidx, float *csrValue, float *VectorX, float *GoldenY)
{
...
void CSC_SpMV(int rowA, int colA, int *cscPtr, int *cscRowidx, float *cscValue, float *VectorX, float *ResultY)
{
...
void ResultVerify(float *resultA, float *resultB, int length, int &result)
{
