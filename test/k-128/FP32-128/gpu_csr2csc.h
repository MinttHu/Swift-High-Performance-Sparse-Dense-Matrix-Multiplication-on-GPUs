...
void csr_to_csc(float &time_csr2csc, int m, int n, int nnz,
                const float* csr_val, const int* csr_row_ptr, const int* csr_col_ind,
                float* csc_val, int* csc_row_ind, int* csc_col_ptr, int* nnzpercol)
{
    float* d_csr_val;
    int* d_csr_row_ptr;
    int* d_csr_col_ind;
    ...
    //CHECK_CUDA (cudaMalloc((void**)&d_nnzpercoltest, (n+1) * sizeof(int)))
    ...
    cusparseCsr2cscEx2_bufferSize(handle, m, n, nnz, d_csr_val, d_csr_row_ptr, d_csr_col_ind,
                                  d_csc_val, d_csc_col_ptr, d_csc_row_ind, CUDA_R_32F, CUSPARSE_ACTION_NUMERIC,
    ...
    //printf("------------------------------------------time_csr2csc %f \n ", time_csr2csc) ;


    //const int numBlocks1 = (n+ blockSize ) / blockSize;
    //prefixSumKernel<<<numBlocks1, blockSize>>>(d_nnzpercol, d_nnzpercoltest, n+1);

/*
    int testtt = 0;
    for(int i =0 ;i <=n; i++)
    {
        if(d_nnzpercoltest[i] != d_csc_col_ptr[i])
        {testtt++;}
    }
    if(testtt!=0)
    {printf("error------------------------------------------------\n");}
...
*/
    ...
    CHECK_CUDA(cudaMemcpy(csc_val, d_csc_val, nnz * sizeof(float), cudaMemcpyDeviceToHost))
    ...
    //CHECK_CUDA(cudaMemcpy(testptr, d_nnzpercoltest, (n + 1) * sizeof(int), cudaMemcpyDeviceToHost))
