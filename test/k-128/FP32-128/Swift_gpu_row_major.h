                                    ...
                                    float *d_resultmtx)
{
    ...
    __shared__ float s_x[warpperblock *slidesize];
    __shared__ float s_y[warpperblock *slidesize];
    __shared__ float s_s[warpperblock *slidesize];

    const int local_warp_id = threadIdx.x >> 5;
    const int lane_id = (TILESIZE -1) & threadIdx.x;
    //const int lane_id = threadIdx.x %32;
    float *s_x_warp = &s_x[local_warp_id *slidesize];
    float *s_y_warp = &s_y[local_warp_id *slidesize];
    float *s_s_warp = &s_s[local_warp_id *slidesize];
    ...
    if(th_tile < tilenum)
    {
        ...
        if(lane_id <tilelen)
        {
            ...
            {
                float d_sp_value = d_sortval[j];
                float d_dn_value = d_densemtx[(tile_colidx+lane_id) * width + ii];
                                            ...
                                            float *d_resultmtx)
{
    ...
    if(th_tile < d_numblk)
    {
        ...
        for(int i = start + lane_id; i< stop; i+=32)
        {
            ...
            {
                float d_dn_value = d_densemtx[real_colidx * width +j];
...
void FastLoad_gpu2(char *filename,
...
else if(tilenum ==0 && h_reside_nnz !=0)
{
    ...
    {
        printf("Swift GPU SpMM (dense row major,with balance) Check NO PASS! error = %d \n", error_FastLoad_spmm);
        time_fastload_gpu = -1;
    }
