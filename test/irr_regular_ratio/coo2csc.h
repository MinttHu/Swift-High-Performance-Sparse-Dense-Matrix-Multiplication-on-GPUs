...
{  
    ...
    {
        printf("error: tmp2: %d tile_ptr: %d nnz: %d \n", tmp2, tile_ptr[tmp2], nnz);
    }
    else
    {
...
void tile_classification(char *filename,
                         slide_matrix *matrix,
...
{  
    ...
    for(int i =0;i<tilenumc;i++)
    {
        ...
        //if(flag ==0 &&length_tmp !=32 && length_tmp !=1)
        //{
           // tile_formatc[i]=2;
...
//------------------------------------------------------------------------------------------------------
    ...
    for(int i =0; i< tilenumc; i++)
    {
        int num = countrow[i+1] - countrow[i];
        int length = tile_len_tmpc[i];
        if(length != 32)
        {
            irregRow ++;
        }
        else if(length ==32 && num == 1)
        {
            allSameRow ++;
        }
        else if(length ==32 && num ==32)
        {
            allNonRow ++;
        }
        else if(length ==32 && num !=32 && num!=1)
        {
            partSameRow++;
        }
    }
    
    //printf("1:%d,2:%d,3:%d,4:%d \n", irregRow, allSameRow, allNonRow, partSameRow);
    int verify = irregRow + allSameRow + allNonRow + partSameRow;
    if(verify == tilenumc)
    {
        printf("success Classification\n");
        regularpart = allSameRow + allNonRow + partSameRow;
        irregularpart = irregRow;
    }
    else
    {
        printf("error Classification-verify:%d right:%d-\n", verify, countrow[tilenumc]);
    }
    double claPercent1 =  static_cast<double>(irregRow) / verify;
    double claPercent2 =  static_cast<double>(allSameRow) / verify;
    double claPercent3  =  static_cast<double>(allNonRow) / verify;
    double claPercent4 =  static_cast<double>(partSameRow) / verify;
