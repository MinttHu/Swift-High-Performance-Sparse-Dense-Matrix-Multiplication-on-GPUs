                          ...
                          float *d_resultmtx)
{
    ...
    float *val_sh = (float *)&sh[(blockDim.y << 6)];
    float *tmp_val_sh = (float *)&sh[(blockDim.y << 7)];
    ...
    if(rid < tilenum)
    {
        ...
        for(int kk = 0; kk < 32; kk++)
        {
            ...
            if(val_offset_sh[thread_idx] != 0 && val_offset_sh[thread_idx] != -1 )
            {
                ...
                for(int i = 0 ; i <=val_offset_sh[thread_idx]; i++)
                {
            ...
            if(val_offset_sh[thread_idx] != 0)
            {
                ...
                float added_val = val_sh[thread_idx];
...
//    cudaStream_t stream1, stream2;
 ...
 //   cudaStreamCreate(&stream2);    
    ...
    if(tilenum !=0 && h_reside_nnz !=0 && width % 32 ==0 )
    {
        ...
        Swift_GPU_4<<<dim3((tilenum+8-1)/8,(width+31)/32,1),dim3(32,8,1),32*8*2*(sizeof(int)+sizeof(float))+32*8*(sizeof(float)),0>>>(rowA, colA, 
                                               width, nnz,
                                               tilenum,
                                               d_tile_ptr,
                                               d_tile_colidx,
    ...
    else if(tilenum !=0 && h_reside_nnz ==0 && width % 32 ==0)
    {
        ...
        Swift_GPU_4<<<dim3((tilenum+8-1)/8,(width+32)/32,1),dim3(32,8,1),32*8*2*(sizeof(int)+sizeof(float))+32*8*(sizeof(float)) , 0>>>(rowA, colA, 
                                               width, nnz,
                                               tilenum,
                                               d_tile_ptr,
                                               d_tile_colidx,
    ...
    else if(tilenum !=0 && h_reside_nnz !=0 && width % 32 !=0 && width /32 == 0)
    {
        ...
        Swift_GPU_4_1<<<dim3((tilenum+8-1)/8,(width+31)/32,1),dim3(32,8,1),32*8*2*(sizeof(int)+sizeof(double)),0>>>(rowA, colA,          
                                               width, nnz,
                                               tilenum,
                                               d_tile_ptr,
                                               d_tile_colidx,
    ...
    else if(tilenum !=0 && h_reside_nnz !=0 && width % 32 !=0 && width /32 != 0)
    {
        ...
        //Swift_GPU<<<tilenum, 1024,32*width*sizeof(double)>>>(rowA, colA,
        //Swift_GPU_3<<<tilenum, 1024>>>(rowA, colA, 
        Swift_GPU_4_2<<<dim3((tilenum+8-1)/8,(width+31)/32,1),dim3(32,8,1),32*8*2*(sizeof(int)+sizeof(double)),0>>>(rowA, colA,          
                                               width, nnz,
                                               tilenum,
                                               d_tile_ptr,
                                               d_tile_colidx,
    ...
    {
        printf("Swift GPU SpMM 4 Check NO PASS! error = %d \n", error_Swift_spmm);
        time_Swift_gpu = -1;
    }
