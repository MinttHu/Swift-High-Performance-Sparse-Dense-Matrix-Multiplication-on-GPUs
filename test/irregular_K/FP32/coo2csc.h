...
void csr2csc(int nnz,
             ...
             float **cscval)
{
    float *cscval_tmp = (float *)malloc(sizeof(float)*nnz);
    memset(cscval_tmp,0,sizeof(float)*(nnz));
    ...
    float csclast = cscval_tmp[nnz-1];
    float csrlast = csrval[nnz-1];
    if(csclast !=csrlast)
    {
...
void formattransation(float time_irrpart,
                      slide_matrix *matrix,
                      int *sortrowidx_tmp,
                      float *sortval_tmp,
                      int *sortnnz_tmp,
                      ...
                      int colA)
{
    ...
    matrix->sortval = (float *)malloc(nnz_regu * sizeof(float));
    memset(matrix->sortval,0,nnz_regu*sizeof(float));
...
gettimeofday(&tirre1, NULL);
    ...
    matrix->reside_val = (float *)malloc((numreside_tmp)*sizeof(float));
    memset(matrix->reside_val,0,(numreside_tmp)*sizeof(float));
    ...
    float *reside_val = matrix->reside_val;
    ...
    double reside_ratio =  (double) count / (double) nnz ;
